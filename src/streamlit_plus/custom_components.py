...
def _validate_css_key(key: str) -> str:
    ...
    Some components emit a ``.st-key-<key>`` selector through
    ``unsafe_allow_html``. Restricting the key to alphanumerics, underscores
    and hyphens prevents callers from breaking out of the ``<style>`` context
    (a CSS/HTML injection vector) and matches Streamlit's key conventions.
    Keys generated by :func:`_default_key` already satisfy this.
    """
...
export default function(component) {
    ...
    // Icons/carets use the Material Symbols font that Streamlit already
    // loads for the host app; no external stylesheet is fetched here.
...
def tree(
    ...
    *,
    ...
    Args:
        ...
        selected_node: Optional id of the node to highlight initially. When
            ``expanded`` is ``False``, only the hierarchy leading to this node
            is expanded.
...
export default function(component) {
    ...
    // Filter/sort/toolbar icons use the Material Symbols font that Streamlit
    // already loads for the host app; no external stylesheet is fetched here.
...
def table(
    ...
    *,
    ...
    current_page = (
        component_state.get("page", 0)
        if isinstance(component_state, dict)
        else getattr(component_state, "page", 0)
    )
...
export default function(component) {
    const { data, parentElement, setTriggerValue } = component;
    parentElement.querySelectorAll(".stplus-icon-wrapper")
        .forEach((el) => el.remove());
...
export default function(component) {
    const { data, parentElement, setTriggerValue } = component;
    parentElement.querySelectorAll(".tile-header")
        .forEach((el) => el.remove());
    ...
    // The tile icon uses the Material Symbols font that Streamlit already
    // loads for the host app; no external stylesheet is fetched here.
    ...
    if (tileContainer) {
        const originalBackgroundColor = tileContainer.style.backgroundColor;
        const onTileEnter = () => {
            const secondaryBackgroundColor = getComputedStyle(
                parentElement.host ?? parentElement
            ).getPropertyValue("--st-secondary-background-color").trim();
...
export default function(component) {
    const { data, parentElement, setTriggerValue } = component;
    parentElement.querySelectorAll(".cal").forEach((el) => el.remove());
    ...
    // Navigation/dot icons use the Material Symbols font that Streamlit
    // already loads for the host app; no external stylesheet is fetched here.
    parentElement.appendChild(root);
